package com.radex392.temporalrelativity.entity.models;
...
public class ModelTimeChicken extends ModelBase
{
    ...
    {
        this.setRotationAngles(v, v1, v2, v3, v4, v5, entity);

        if (this.isChild)
        {
            float f6 = 2.0F;
            GL11.glPushMatrix();
            GL11.glTranslatef(0.0F, 5.0F * v5, 2.0F * v5);
            this.head.render(v5);
            this.bill.render(v5);
            this.chin.render(v5);
            GL11.glPopMatrix();
            ...
            GL11.glTranslatef(0.0F, 24.0F * v5, 0.0F);
            this.body.render(v5);
            this.rightLeg.render(v5);
            this.leftLeg.render(v5);
            this.rightWing.render(v5);
            this.leftWing.render(v5);
            GL11.glPopMatrix();
        }
        else
        {
    ...
    public void setRotationAngles(float v, float v1, float v2, float v3, float v4, float v5, Entity entity)
    {
        ...
        this.rightLeg.rotateAngleX = MathHelper.cos(v * 0.6662F) * 1.4F * v1;
        this.leftLeg.rotateAngleX = MathHelper.cos(v * 0.6662F + (float)Math.PI) * 1.4F * v1;
        this.rightWing.rotateAngleZ = v2;
        this.leftWing.rotateAngleZ = -v2;
    }
}